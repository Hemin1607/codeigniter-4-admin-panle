...
class HomeModel extends Model
{
	...
	protected $allowedFields = [ 'title','description','updated','image'];
	var $order = array('id' => 'asc');
	function get_datatables(){
		// search
		if($_POST['search']['value']){
			$search = $_POST['search']['value'];
			$condition_search = "title LIKE '%$search%' OR description LIKE '%$search%'";
		} else {
			$condition_search = "Id != ''";
		}
    ...
    function count_filter(){
		if($_POST['search']['value']){
			$search = $_POST['search']['value'];
            $condition_search = "AND  (description LIKE '%$search%' OR title LIKE '%$search%')";
		} else {
			$condition_search = "";
		}
